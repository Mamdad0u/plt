...
BOOST_AUTO_TEST_CASE(NewTestSurface){
  {
    render::Surface lBackgroundSurface;
    render::Surface lCharacterSurface;
    ...
    lCharacterSurface.ResetSpritePosition(0);
    lBackgroundSurface.ResetViewPosition();

    lCharacterSurface.DrawSprite(window);
    lBackgroundSurface.DrawSprite(window);
...
BOOST_AUTO_TEST_CASE(TestCommand){
  {
    ...
    int PVLost;
    PVLost=0;
    PVLost=lCommand.ComputeAction(Character1,Character2,client::ATTACK_1);
...
BOOST_AUTO_TEST_CASE(TestPlayer){
  ...
  Engine lEngine;
  sf::RenderWindow window; 
  WindowCursor lCursor;

  lPlayer.AddEngineObserver(&lEngine);
  lPlayer.SetStatusCommand(CommandID::ATTACK_1);

  BOOST_CHECK_EQUAL(lPlayer.GetStatusCommand(),CommandID::ATTACK_1);

  lPlayer.ClickCommand(window, lCursor);
 // lPlayer.DelEngineObserver(&lEngine);
}
