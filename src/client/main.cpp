...
void main_render(void) {
  ...
  while (window.isOpen()) {
    ...
    if ((sf::Mouse::getPosition(window).y > 550) &&
        ...
        (sf::Mouse::getPosition(window).x > 35) &&
...
void main_engine(void) {
  ...
  CommandID PlayerCommand[] = {client::ATTACK_1, client::ATTACK_1,
                               client::ATTACK_1};
  ...
  while (window.isOpen()) {
    ...
    if (GameClock.getElapsedTime().asSeconds() > 3.f) {
      ...
      turn++;
      GameStatus = GameEngine.GameLoop(); // Update de l'état du jeu toutes
                                          // les 3s pour + de visibilité
      GameClock.restart();
    }
...
int main(int argc, char *argv[]) {
  ...
  if (argc > 1) {
    if (strcmp(argv[1], "hello") == 0) {
      ...
      state::JSON_tools jtools = JSON_tools();
    }
  }
